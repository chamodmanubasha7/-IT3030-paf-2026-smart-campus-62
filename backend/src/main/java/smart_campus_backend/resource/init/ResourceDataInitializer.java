...
public class ResourceDataInitializer implements CommandLineRunner {
    ...
    private void seedUsers() {
        // We check for each user individually so we can add them even if some exist
        seedUser("Student", "it23123456@my.sliit.lk", "12345678", Role.USER);
        seedUser("Admin User", "admin.user@gmail.com", "abcd1234", Role.ADMIN);
        seedUser("Technician Demo", "technician.demo@my.sliit.lk", "tech1234", Role.TECHNICIAN);
        seedUser("Super Admin", "super.admin@smartcampus.lk", "super1234", Role.SUPER_ADMIN);

        // Keep the old demo users if needed, but the user requested these specific ones
        seedUser("Campus Admin", "admin@smartcampus.lk", "password", Role.ADMIN);
        seedUser("John Doe", "john_doe@gmail.com", "password", Role.USER);
    }
